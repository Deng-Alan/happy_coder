...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('happy auth')} - Personal device pairing

${chalk.bold('Usage:')}
  happy auth login [--force]    Pair this CLI with your personal server
  happy auth logout             Remove local pairing and machine data
  happy auth status             Show current pairing status
  happy auth help               Show this help message

${chalk.bold('Options:')}
  --force    Clear local credentials, machine ID, and stop daemon before re-pairing
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (forceAuth) {
    console.log(chalk.yellow('Force re-pair requested.'));
    console.log(chalk.gray('This will:'));
    console.log(chalk.gray('  • Clear existing local credentials'));
    console.log(chalk.gray('  • Clear machine ID'));
    console.log(chalk.gray('  • Stop daemon if running'));
    console.log(chalk.gray('  • Pair this machine again with your personal setup\n'));
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'happy auth login --force' to re-pair this machine`));
      return;
    } else if (existingCreds && !settings?.machineId) {
      console.log(chalk.yellow('⚠️  Local credentials exist but machine ID is missing'));
      console.log(chalk.gray('  Fixing local machine setup...\n'));
    }
  }
  ...
  try {
    ...
    console.error(chalk.red('Pairing failed:'), error instanceof Error ? error.message : 'Unknown error');
    process.exit(1);
  }
}
...
async function handleAuthLogout(): Promise<void> {
  ...
  if (!credentials) {
    console.log(chalk.yellow('Not currently paired'));
    return;
  }

  console.log(chalk.blue('This will remove this machine from your local personal setup'));
  console.log(chalk.yellow('⚠️  You will need to pair again before using Happy on this machine'));
  ...
  const answer = await new Promise<string>((resolve) => {
    rl.question(chalk.yellow('Are you sure you want to remove local pairing? (y/N): '), resolve);
  });
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      console.log(chalk.green('✓ Local pairing removed'));
      console.log(chalk.gray('  Run "happy auth login" to pair again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "happy auth login" to pair this machine'));
    return;
  }
