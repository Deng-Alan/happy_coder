...
import { DISABLED_PERSONAL_COMMANDS, getDisabledCommandMessage } from './personalEdition'
...
(async () => {
  ...
  if (subcommand && DISABLED_PERSONAL_COMMANDS.has(subcommand)) {
    printDisabledPersonalCommand(subcommand)
    process.exit(1)
  }
...
${chalk.bold('Usage:')}
  ...
  happy daemon install            Install login autostart
  happy daemon uninstall          Remove login autostart
...
${chalk.bold('Usage:')}
  happy [options]         Start Claude on your self-hosted setup
...
${chalk.bold('Disabled In Personal Edition:')}
  happy gemini
  happy acp
  happy openclaw
  happy connect
...
function printDisabledPersonalCommand(command: string) {
  console.error(chalk.yellow(getDisabledCommandMessage(command)))
}
