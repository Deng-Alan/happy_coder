...
export async function getConnectedServices(credentials: AuthCredentials): Promise<string[]> {
    const API_ENDPOINT = getServerUrl();

    return await backoff(async () => {
        const response = await fetch(`${API_ENDPOINT}/v1/connect/tokens`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${credentials.token}`,
                'Content-Type': 'application/json'
        ...
        if (!response.ok) {
            throw new Error(`Failed to load connected services: ${response.status}`);
        }

        const data = await response.json() as { tokens: Array<{ vendor: string }> };
        return data.tokens.map((item) => item.vendor);
    });
}
