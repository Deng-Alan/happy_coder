...
describe('ops v3 routing', () => {
    ...
    it('falls back to legacy RPC when a v3 permission request is not available', async () => {
        await sessionAllow('ses_1', 'perm_1');

        expect(fetchSession).toHaveBeenCalledWith('ses_1');
        expect(sessionRPC).toHaveBeenCalledWith('ses_1', 'permission', {
            id: 'perm_1',
            approved: true,
            decision: 'approved',
        });
    ...
    it('falls back to legacy RPC when SyncNode approval fails', async () => {
        getSession.mockReturnValue({
            permissions: [{ permissionId: 'perm_1', resolved: false }],
            questions: [],
        });
        approvePermission.mockRejectedValueOnce(new Error('sync unavailable'));

        await sessionAllow('ses_1', 'perm_1', 'acceptEdits');
    ...
    it('falls back to legacy RPC when SyncNode denial fails', async () => {
        getSession.mockReturnValue({
            permissions: [{ permissionId: 'perm_1', resolved: false }],
            questions: [],
        });
        denyPermission.mockRejectedValueOnce(new Error('sync unavailable'));
