...
interface LegacyPermissionResponse {
    id: string;
    approved: boolean;
    reason?: string;
...
function buildLegacyApprovalPayload(
    id: string,
    mode?: 'default' | 'acceptEdits' | 'bypassPermissions' | 'plan',
    allowedTools?: string[],
    decision?: 'approved' | 'approved_for_session',
): LegacyPermissionResponse {
    return {
        id,
        approved: true,
        ...(mode ? { mode } : {}),
        ...(allowedTools && allowedTools.length > 0 ? { allowTools: allowedTools } : {}),
        decision: decision ?? ((allowedTools?.length ?? 0) > 0 ? 'approved_for_session' : 'approved'),
    };
}

function buildLegacyDenialPayload(
    id: string,
    decision?: 'denied' | 'abort',
): LegacyPermissionResponse {
    const isAbort = decision === 'abort';
    return {
        id,
        approved: false,
        reason: isAbort ? 'request aborted by user' : 'request denied by user',
        decision: isAbort ? 'abort' : 'denied',
    };
}

async function sendLegacyPermissionResponse(
    sessionId: string,
    payload: LegacyPermissionResponse,
): Promise<void> {
    await apiSocket.sessionRPC<void, LegacyPermissionResponse>(sessionId, 'permission', payload);
}
...
export async function sessionAllow(sessionId: string, id: string, mode?: 'default' | 'acceptEdits' | 'bypassPermissions' | 'plan', allowedTools?: string[], decision?: 'approved' | 'approved_for_session'): Promise<void> {
    ...
    if (pendingPermission && sync.appSyncStore) {
        try {
            ...
            console.warn(`Falling back to legacy permission approval for session ${sessionId}:`, error);
        }
    }

    await sendLegacyPermissionResponse(
        sessionId,
        buildLegacyApprovalPayload(id, mode, allowedTools, decision),
    );
}
...
export async function sessionDeny(sessionId: string, id: string, mode?: 'default' | 'acceptEdits' | 'bypassPermissions' | 'plan', allowedTools?: string[], decision?: 'denied' | 'abort'): Promise<void> {
    ...
    if (pendingPermission && sync.appSyncStore) {
        try {
            ...
            console.warn(`Falling back to legacy permission denial for session ${sessionId}:`, error);
        }
    }

    await sendLegacyPermissionResponse(
        sessionId,
        buildLegacyDenialPayload(id, decision),
    );
}
