...
function ResumeSessionButton(props: {
    canResume: boolean;
    onPress: () => void;
    resumingSession: boolean;
}) {
    const { theme } = useUnistyles();
    const disabled = !props.canResume || props.resumingSession;
    ...
    return (
        <Pressable
            ...
            style={{
                minHeight: 48,
                borderRadius: 14,
                backgroundColor: disabled ? theme.colors.surfaceHigh : '#007AFF',
                flexDirection: 'row',
                alignItems: 'center',
                justifyContent: 'center',
                gap: 8,
        ...
        >
            {props.resumingSession ? (
                <ActivityIndicator size="small" color={disabled ? theme.colors.textSecondary : '#FFFFFF'} />
            ) : (
                <Ionicons
